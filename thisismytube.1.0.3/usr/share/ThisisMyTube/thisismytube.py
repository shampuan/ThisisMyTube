...
import os
# Wayland/KDE/GNOME uyumluluğu: Hem Qt hem de MPV'yi kesin olarak X11/XWayland katmanına zorla
os.environ["QT_QPA_PLATFORM"] = "xcb"
# MPV'nin arka planda Wayland'e bağlanıp Qt'nin X11 pencere kimliğini reddetmesini engelle:
...
class ThumbnailWorker(QRunnable):
    ...
    def run(self):
        if not os.path.exists(self.thumb_path):
            try:
                # 6.6 kat daha az CPU tüketen, anahtar kareye (keyframe) zıplayan hızlı komut:
                cmd = [
                    "ffmpeg", "-y",
                    "-ss", "00:00:05",
                    "-noaccurate_seek",
...
class FolderExplorerBox(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.breadcrumb_label.setStyleSheet("color: #a0a0a0; font-size: 9pt;")
    ...
    def refresh(self):
        ...
        # Dosyalar sadece bilgi amaçlı gösteriliyor: tıklanamaz/seçilemez
        # Koyu temada da net okunabilir yumuşak bir gri tonu veriyoruz
        file_icon = style.standardIcon(QStyle.StandardPixmap.SP_FileIcon)
        for name in files:
            item = QListWidgetItem(file_icon, name)
            item.setFlags(item.flags() & ~Qt.ItemFlag.ItemIsSelectable)
            item.setForeground(QColor(165, 165, 165))
            self.folder_list.addItem(item)
...
class AspectRatioWidget(QWidget):
    ...
    def apply_geometry(self):
        ...
        if comment_area is None or comment_area.isVisible():
            # Header (105px) + Başlık (35px) + Seek Bar (38px) + Butonlar (48px) + 
            # Puan/İzlendi (36px) + Notlar (95px) + KDE Spacing/Margin (~40px)
            reserved_for_controls = 395  # Normal pencere modu (KDE/GNOME uyumlu)
        else:
            ...
            reserved_for_controls = 65 if getattr(top_level, '_fullscreen_bar_visible', False) else 0
...
class ThisIsMyTubeApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # --- UI Kurulumu (Önce tüm layout hiyerarşisi kurulmalı) ---
        ...
        # MPV alanını oluşturup doğrudan layout'a bağlıyoruz
        ...
        # Pencereyi anında (gecikmesiz) ekrana aç; MPV ve verileri
        # pencere çizildikten hemen sonra (arka planda) devreye sok:
        QTimer.singleShot(0, self._init_mpv_player)
    ...
    def _start_disk_scan(self):
        ...
        raw_base = self.base_dirs_line_edit.text() if hasattr(self, 'base_dirs_line_edit') else ""
        base_dir_text = self._normalize_path(raw_base)
        if not base_dir_text or not os.path.isdir(base_dir_text):
            self.scan_results_label.setText(_("Hata: Lütfen geçerli bir taranacak ana dizin belirtin."))
            return

        def _get_valid_dir(attr):
            if hasattr(self, attr):
                raw = getattr(self, attr).text().strip()
                if raw:
                    p = self._normalize_path(raw)
                    if os.path.isdir(p):
                        return p
            return None

        # Dizinleri tarama başlamadan önce SADECE 1 KEZ normalize ediyoruz (hız kazanımı)
        mode_dir_map = {}
        for mode_name, attr in [
            ...
            val = _get_valid_dir(attr)
            if val:
                mode_dir_map[mode_name] = val

        total_files = 0
        total_videos = 0

        self.scan_results_label.setText(_("Tarama Başlatıldı... Lütfen bekleyin."))
        QApplication.processEvents()

        # Ana dizinin dışında kalan harici mod kökleri
        extra_roots = []
        for mode_name, mdir in mode_dir_map.items():
            if not self._is_subpath_of(mdir, base_dir_text):
        ...
        def _scan_root(root_dir, forced_mode=None):
            ...
            for root, _, files in os.walk(root_dir):
                ...
                if forced_mode:
                    ...
                    for mode_name, mdir in mode_dir_map.items():
                        try:
                            if os.path.commonpath([abs_root, mdir]) == mdir:
        ...
        # Arka planda verileri hazırla ama ekranda kal, kullanıcı sonuçları okusun
        ...
        if self.found_videos:
            QMessageBox.information(
                ...
                _("Toplam {count} video bulundu ve kaydedildi!").format(count=total_videos)
            )
    ...
    def _on_folder_dump_requested(self, folder_path):
        ...
        norm_folder = self._normalize_path(folder_path)
        ...
        videos = [v for v in active_videos if self._is_subpath_of(v.get('path', ''), norm_folder)]

        folder_name = os.path.basename(norm_folder) or norm_folder
    ...
    def _normalize_path(p):
        """Yolları tilde (~), göreceli yollar ve sembolik bağlardan (symlink) arındırır."""
        if not p:
            return ""
        return os.path.realpath(os.path.abspath(os.path.expanduser(str(p).strip())))

    @classmethod
    def _is_subpath_of(cls, target_path, parent_dir):
        """target_path'in parent_dir dizininin kendisi veya bir alt öğesi olup olmadığını güvenle denetler."""
        if not target_path or not parent_dir:
            return False
        try:
            t = cls._normalize_path(target_path)
            p = cls._normalize_path(parent_dir)
            return os.path.commonpath([t, p]) == p
        except Exception:
            return False
    ...
    def _on_mode_changed(self, mode):
        ...
        # Mod değiştirildiğinde kullanıcıyı doğrudan o modun videolarını içeren Ana Sayfa'ya al
        self.active_menu_id = "home"
        self._update_menu_style()
        self.stacked_widget.setCurrentIndex(0)
    ...
    def _init_mpv_player(self):
        """MPV'yi tüm pencere hiyerarşisi kilitlendikten sonra X11 context'iyle başlatır."""
        t_mpv = time.time()
        print("[MPV] Başlatılıyor...")

        wid_val = int(self.mpv_area_widget.winId())
        ...
        print(f"[MPV] Tamamlandı! Geçen süre: {time.time() - t_mpv:.3f} sn")
    ...
    def _setup_mpv_widget(self):
        ...
        # KWin'in 5 saniyelik senkronizasyon kilitlenmesini engelleyen temiz X11 pencere ayarı:
        self.mpv_area_widget.setAttribute(Qt.WidgetAttribute.WA_NativeWindow, True)
        self.mpv_area_widget.setAttribute(Qt.WidgetAttribute.WA_DontCreateNativeAncestors, True)
        return player_area
    ...
    def _create_player_view(self, player_area): 
        ...
        self.video_notes_edit.setStyleSheet(
            "QTextEdit {"
        ...
        pal_notes = self.video_notes_edit.palette()
        pal_notes.setColor(pal_notes.ColorRole.PlaceholderText, QColor(160, 160, 160))
        self.video_notes_edit.setPalette(pal_notes)
    ...
    def _load_persistent_data(self):
        ...
        if hasattr(self, 'folder_explorer'):
            self.folder_explorer.set_root(self._get_mode_root_dir())
        print(f"[1] Klasör Gezgini (FolderExplorer): {time.time() - t1:.3f} sn")
        ...
        if os.path.exists(self.index_file):
            ...
            self.found_videos = []
        print(f"[2] JSON Okuma ({len(self.found_videos)} video): {time.time() - t2:.3f} sn")

        t3 = time.time()
        if hasattr(self, 'home_video_grid'):
            self.home_video_grid.set_videos(self._filtered_videos_for_mode(), self.THUMB_PATH)
        print(f"[3] Ana Grid Kartları (home_video_grid): {time.time() - t3:.3f} sn")

        t4 = time.time()
        self._update_recommended_videos()
        print(f"[4] Önerilen Videolar (sidebar): {time.time() - t4:.3f} sn")

        t5 = time.time()
        self._update_mode_info_label()
        self._update_watched_and_favorites_lists()
        print(f"[5] İzlenenler/Favoriler Listesi: {time.time() - t5:.3f} sn")

        print(f"--- TOPLAM YÜKLEME SÜRESİ: {time.time() - t_start:.3f} sn ---\n")
    ...
    def _create_header(self):
        ...
        self.search_input.setStyleSheet(
            ...
            "   font-size: 10pt;"
            "}"
            "QLineEdit:focus { border: 1.5px solid #007acc; }"
        )
        pal = self.search_input.palette()
        pal.setColor(pal.ColorRole.PlaceholderText, QColor(160, 160, 160))
    ...
    def _create_home_view(self):
        ...
        # Kartların kontrolsüzce üst üste binmesini önleyen korumalı yükleyici:
        def _check_scroll_bottom(value):
            max_val = scroll_area.verticalScrollBar().maximum()
            if max_val > 0 and value >= max_val - 200:
                # Halihazırda kartlar işleniyorsa bekle, kartları üst üste bindirme
                if not getattr(self.home_video_grid, '_is_loading', False):
                    self.home_video_grid.load_more(count=24)
    ...
    def _create_disk_scan_settings_view(self):
        ...
        # KDE Koyu Tema ve XFCE uyumlu net QLineEdit stili
        line_edit_style = (
            "QLineEdit {"
            "   background-color: palette(base);"
            "   color: palette(text);"
        ...
        for label_text, attr_name, placeholder in dir_fields:
            ...
            # Placeholder metninin koyu temada siyah içinde kaybolmasını önle:
    ...
    def _create_about_view(self):
        ...
        meta_info = QLabel(
            ...
            f"<tr><td><b>{_('Sürüm:')}</b></td><td style='padding-left: 12px;'>1.0.3</td></tr>"
